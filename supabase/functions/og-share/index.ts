...
Deno.serve(async (req) => {
  ...
  const SITE_ORIGIN = "https://sudagospel.net";

  let title = "Suda Gospel";
  ...
  if (type === "song") {
    ...
    if (song) {
      ...
      description = song.description || `Listen to ${song.title} by ${artistName} on Suda Gospel. ${song.genre || "Gospel"} music from South Sudan.`;
    ...
    if (artist) {
      title = `${artist.name} - Suda Gospel`;
      description = artist.bio || `Listen to ${artist.name}'s gospel music on Suda Gospel. ${artist.genre || "Gospel"} artist from South Sudan.`;
    ...
    if (article) {
      title = `${article.title} - Suda Gospel`;
      description = article.excerpt || "Read the latest gospel news on Suda Gospel.";
...
<head>
  ...
  <meta property="og:site_name" content="Suda Gospel">
