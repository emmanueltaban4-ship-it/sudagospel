    ...
    <Layout>
      <div className="min-h-screen pb-24">
        ...
        <div className="px-4 lg:px-6 pt-6 pb-2">
          <h1 className="font-heading text-2xl md:text-3xl font-extrabold text-foreground tracking-tight mb-4">Explore</h1>
        ...
        {trending.length > 0 && !search && (
          <div className="px-4 lg:px-6 py-5">
            <div className="flex items-center gap-2.5 mb-4">
              ...
              <h2 className="font-heading text-lg font-extrabold text-foreground tracking-tight">Trending</h2>
              <span className="text-xs text-muted-foreground ml-auto">{allSongs.length} songs</span>
            </div>
            <div className="rounded-xl border border-border bg-card/50 overflow-hidden">
              {trending.map((song, index) => {
                ...
                return (
                  <div
                    key={song.id}
                    className={`flex items-center gap-3 px-4 py-3 transition-colors cursor-pointer group ${isCurrent ? "bg-primary/5" : "hover:bg-muted/40"} ${index < trending.length - 1 ? "border-b border-border" : ""}`}
                    onClick={() => handlePlayTrack(song)}
                  >
                    <span className={`text-sm font-extrabold w-7 text-center tabular-nums ${index < 3 ? "text-primary" : "text-muted-foreground/50"}`}>
                      {index + 1}
                    </span>
        ...
        {newReleases.length > 0 && !search && (
          <div className="px-4 lg:px-6 py-5">
            <div className="flex items-center gap-2.5 mb-4">
              ...
              <h2 className="font-heading text-lg font-extrabold text-foreground tracking-tight">Fresh Releases</h2>
            </div>
              ...
              {newReleases.map((song) => {
                ...
                return (
                  <div key={song.id} className="flex-shrink-0 w-36 md:w-44 group cursor-pointer" onClick={() => handlePlayTrack(song)}>
                    <div className="relative aspect-square rounded-lg overflow-hidden mb-2.5 bg-muted shadow-sm">
                      {song.coverUrl ? (
                        <img src={song.coverUrl} alt={song.title} className="h-full w-full object-cover group-hover:scale-105 transition-transform duration-300" loading="lazy" />
                      ) : (
                        <div className="h-full w-full flex items-center justify-center"><Music className="h-8 w-8 text-muted-foreground" /></div>
                      )}
                      <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                        <div className="h-11 w-11 rounded-full bg-primary flex items-center justify-center shadow-xl">
